...
- PR-B deployment recovery hardening tại `0e82af292320875471495768bc0ee8e6154b0abb`: installer theo dõi explicit phase và ghi non-secret `IisReleaseInstallFailed` evidence. Sau khi migration invocation bắt đầu, failure không bao giờ tự restart release cũ; pool được giữ/dừng lại để operator review migration/schema compatibility và chọn explicit release switch, forward fix hoặc verified DB recovery. Chỉ failure trước migration với schema untouched và IIS path cũ được chứng minh mới có thể resume existing app. PowerShell parser `11/11`, injected pre-/post-migration failure paths, secret-boundary checks và full regression đều pass; `Switch-SimpleStoreRelease.ps1 -SchemaCompatibilityReviewed` giữ nguyên explicit gate.
- GitHub Actions CI #55 / `36251258900` tại PR-B evidence head `7ddc974763f018f095214edef5be0f7f44132ef1` là `SUCCESS`; backend và frontend đều `SUCCESS`. GitHub CI không chạy real Playwright hoặc restore/IIS drill, nên các kết quả đó vẫn là evidence riêng theo môi trường.
...
2026-09-26 — PR-B deployment recovery hardening tại `0e82af292320875471495768bc0ee8e6154b0abb`: loại bỏ unsafe automatic previous-release restart sau migration attempt; thêm phase/state tracking, fail-safe pool stop, non-secret failure evidence và high-severity reviewed-recovery guidance; deployment runbook phân biệt rõ pre-migration verified resume với post-migration manual recovery. Không sửa `Switch-SimpleStoreRelease.ps1`, không thêm migration, và không thay đổi approval/blocker state. Parser/fault injection/full backend/frontend regression pass. PR-B vẫn `IMPLEMENTED / EVIDENCE INCOMPLETE / PENDING PRODUCT OWNER REVIEW`; PR-BLOCKER-03/04/05 vẫn `OPEN`, PR-C `NOT STARTED`, M7 `NOT ACHIEVED`, Pilot `NOT STARTED`.
