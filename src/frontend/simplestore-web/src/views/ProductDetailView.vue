...
<template>
  <section>
    ...
    <template v-if="product">
      <div class="mt-3 flex flex-wrap items-start justify-between gap-4"><div><h1 class="text-3xl font-black">{{ product.name }}</h1><p class="mt-1 text-slate-500">{{ product.sku }} · {{ product.barcode || 'Không có barcode' }}</p></div><div class="flex gap-2"><RouterLink class="btn-secondary" :to="`/products/${id}/edit`">Sửa</RouterLink><button v-if="product.isActive" class="btn-secondary" :disabled="deactivating" @click="deactivate">Ngừng bán</button></div></div>
      <div class="mt-6 grid gap-4 sm:grid-cols-2 lg:grid-cols-4"><div class="card"><p class="text-sm text-slate-500">Tồn hiện tại</p><p class="mt-1 text-2xl font-black">{{ product.quantityOnHand }} {{ product.unit }}</p></div><div class="card"><p class="text-sm text-slate-500">Giá trị tồn</p><p class="mt-1 text-2xl font-black">{{ money(product.inventoryValue) }} ₫</p></div><div class="card"><p class="text-sm text-slate-500">Giá vốn bình quân</p><p class="mt-1 text-2xl font-black">{{ money(product.averageCost) }} ₫</p></div><div class="card"><p class="text-sm text-slate-500">Giá bán</p><p class="mt-1 text-2xl font-black">{{ money(product.salePrice) }} ₫</p></div></div>
      <div class="card mt-6"><h2 class="text-xl font-black">Nguồn tồn kho</h2><p v-if="movements.length === 0" class="mt-4 text-slate-500">Sản phẩm chưa có tồn đầu.</p><div v-for="movement in movements" :key="movement.id" class="mt-4 flex flex-wrap justify-between gap-3 border-t pt-4"><div><p class="font-semibold">Tồn đầu</p><p class="text-sm text-slate-500">{{ new Date(movement.occurredAt).toLocaleString('vi-VN') }}</p></div><div class="text-right"><p class="font-semibold">+{{ movement.quantityDelta }} {{ product.unit }}</p><p class="text-sm text-slate-500">{{ money(movement.inventoryValueDelta) }} ₫</p></div></div></div>
    </template>
  </section>
</template>
