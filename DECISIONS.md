...
### D-092 — Pilot Readiness Technical Breakdown Approval

- **Trạng thái:** `APPROVED`
- **Ngày:** 2026-09-25
- **Người phê duyệt:** Product Owner
- **Approval:** Product Owner approve [Technical Breakdown Pilot Readiness v0.1](docs/architecture/technical-breakdown-pilot-readiness-v0.1.md) tại reviewed baseline `5e5720659cee1fd400001d2a2f7c5b750e0483b6`, với toàn bộ contracts D-077–D-084 và D-085–D-091 đã được incorporated. Trạng thái tài liệu chuyển từ `DRAFT / PENDING PRODUCT OWNER REVIEW` sang `APPROVED FOR IMPLEMENTATION — D-092`.
- **Approved staging:** PR-A — Functional pilot blockers, PR-B — Operational safety và PR-C — Pilot certification and release gate được approve làm implementation sequence. Cả ba stage có trạng thái `APPROVED FOR IMPLEMENTATION / NOT STARTED`; mỗi stage vẫn cần implementation evidence và Product Owner review/approval riêng trước khi được coi là complete.
- **PR-A scope:** Production first Owner bootstrap; Store-scoped Owner-only Cashier lifecycle; temporary password + mandatory change; restricted pre-change session; disable/reset session invalidation; account lifecycle audit; Stock Adjustment; Stocktake; D-087/D-088 costing; D-089 stale handling; typed `InventoryMovement` additions; migration; API/UI; domain, SQL Server integration, frontend và real E2E verification. PR-A nhắm `PR-BLOCKER-01` và `PR-BLOCKER-02` nhưng chưa đóng blocker nào.
- **PR-B scope:** Windows Server/IIS deployment; artifact/version identification; protected configuration/secrets; explicit migration execution; persistent logs và `traceId` correlation; DB-aware readiness; support runbook; D-090 SQL Server Full recovery, nightly full, 15-minute transaction-log backups, 14-day recovery chain, weekly full retention 8 weeks và actual isolated full+log restore drill. PR-B nhắm `PR-BLOCKER-03`–`PR-BLOCKER-05` nhưng chưa đóng blocker nào.
- **PR-C scope:** D-091 `80 mm` thermal/browser-print certification trên actual printer/browser/driver; release checklist với exact SHA/version, CI, migrations, real E2E, production-like smoke và rollback/recovery; pilot Store onboarding/data preparation; support/contact/escalation; D-084 validation execution plan. PR-C nhắm `PR-BLOCKER-06`, `PR-BLOCKER-07`, Pilot operations gate và Validation readiness gate nhưng chưa đóng gate nào.
- **D-085/D-086 preservation:** First Owner dùng explicit one-shot admin CLI/command; không public bootstrap endpoint, Production `DevelopmentOwnerSeeder`, normal-flow manual SQL hoặc plaintext secret logging. Cashier dùng temporary password + mandatory change; pre-change session chỉ có password-change/logout/minimal auth; reset/disable invalidates sessions và login failures giữ generic.
- **D-087/D-088 preservation:** Positive Adjustment/Stocktake difference dùng reliable current average khi có; nếu tăng tồn mà thiếu reliable basis thì yêu cầu explicit Adjustment Unit Cost và không silent fallback `ReferencePurchaseCost`. Movement cost input không tự động làm toàn bộ balance reliable. Negative delta dùng `Reliable` average, nếu thiếu thì `Estimated` reference cost, nếu vẫn thiếu thì cost `0`/`Unavailable`. Stocktake reuse chính xác cùng rule; không retroactive revaluation hoặc rewrite historical `SaleLine`.
- **D-089 preservation:** Stocktake gửi expected revision/rowversion; stale request nhận typed `409 stocktake-stale` và phải refresh + recount/new submission. Không silent recalculation, apply old difference hoặc overwrite movement mới hơn.
- **D-090/D-091 preservation:** Backup giữ đúng Full recovery/nightly full/15-minute log/14-day recovery chain/weekly full 8 weeks và isolated real restore drill. Printing giữ `80 mm` thermal baseline, browser print default, actual pilot-device certification và không print agent nếu chưa có certification failure cùng approval sau đó.
- **Approval-baseline evidence:** GitHub Actions CI #49 / run `36037545751` tại reviewed baseline `5e5720659cee1fd400001d2a2f7c5b750e0483b6` là `SUCCESS`; backend và frontend đều `SUCCESS`. Đây chỉ là Technical Breakdown approval-baseline evidence, không phải PR-A implementation evidence.
- **Boundary:** D-092 approve Technical Breakdown, architecture, contracts và implementation sequence; không có nghĩa PR-A/PR-B/PR-C đã implement, PR-BLOCKER-01..07 đã đóng, M7 đã đạt, Pilot Ready/Started/Validated hoặc Production Ready. M7 vẫn cần final Product Owner approval riêng sau reviewed evidence.
- **Tiếp theo:** Implement Stage PR-A — Functional pilot blockers theo approved Technical Breakdown; không tự bắt đầu PR-B/PR-C hoặc suy diễn completion.

### D-093 — Pilot Readiness PR-A Approval

- **Status:** `APPROVED / COMPLETED`
- **Date:** `2026-09-26`
- **Approved by:** `Product Owner`
- **Approval:** Product Owner reviewed and approved **PR-A — Functional pilot blockers**. Reviewed implementation/evidence gồm initial PR-A implementation `ceae40ee97da3468954da8e27b852bcbbfa94113`, integrity hardening `d0abe321f198f05890f566adf137844826973e7a`, và final reviewed state/evidence `bf733a6923b2d0b7c2162b37fdcddd7f42643b74`.
- **PR-BLOCKER-01 — Production account provisioning:** Approved evidence gồm production first Owner one-shot CLI; không có public bootstrap endpoint; Development seeder vẫn chỉ chạy trong Development; Owner Store initialization vẫn là bước riêng; Owner-only Store-scoped Cashier management với create/list/disable/reset; one-time temporary password; mandatory password change; backend forced-change enforcement; immediate session invalidation; generic login failure; account lifecycle audit; atomic credential/state/security-stamp/audit transactions; rollback failure-path coverage; authorization và Store isolation. `PR-BLOCKER-01 = CLOSED — D-093`.
- **PR-BLOCKER-02 — Inventory MVP completeness:** Approved evidence gồm Stock Adjustment; Stocktake; immutable source records; typed `Adjustment` và `StocktakeAdjustment` movements; D-087 Adjustment costing; D-088 Stocktake costing; D-089 stale Stocktake behavior; OperationId idempotency; balance locking/concurrency; Store/Main Warehouse scope; Owner-only mutation; zero-difference Stocktake không tạo fake movement; negative counted quantity rejection; exact storage precision guards; không silent SQL rounding; explainable inventory history; migration và upgrade coverage. `PR-BLOCKER-02 = CLOSED — D-093`.
- **Build/test evidence:** Release build `0 warnings / 0 errors`; Domain `96/96`; SQL Server integration `97/97`; frontend `23 test files / 106 tests`; production frontend build `PASS`; real PR-A Playwright `1/1 PASS` qua Vue → ASP.NET Core → temporary LocalDB/SQL Server test environment. GitHub CI hiện không chạy real Playwright suite.
- **GitHub Actions evidence:** Code-hardening CI #52 / run `36231423946` là `SUCCESS`. Final reviewed HEAD CI #53 / run `36231672552` là `SUCCESS`; backend và frontend đều `SUCCESS`.
- **Migration evidence:** Approved PR-A migration là `20260926023259_ImplementPilotReadinessPrA`; integrity hardening không cần migration mới. Evidence gồm clean database migration, upgrade từ Slice 6 baseline, và preservation của existing Identity/Store/Product/InventoryBalance.
- **Boundary:** D-093 không có nghĩa PR-B hoặc PR-C đã complete, PR-BLOCKER-03..07 đã đóng, M7 đã đạt, Pilot Ready/Started/Validated hoặc Production Ready. Giữ nguyên `M7 — NOT ACHIEVED`, `Pilot — NOT STARTED`, `Production readiness — NOT DECLARED`, và `C14 value / willingness-to-pay — NOT VALIDATED`.
- **Tiếp theo:** PR-B — Operational safety là `APPROVED FOR IMPLEMENTATION / NOT STARTED`; không có PR-B implementation hoặc approval decision nào được tạo bởi D-093.
