...
test('Owner completes a purchase with partial payment and sees inventory cost and debt', async ({ page }) => {
  ...
  await page.getByLabel('Nhà cung cấp').selectOption({ label: supplierName })
  const productId = await page.getByLabel('Sản phẩm').locator('option').filter({ hasText: productName }).getAttribute('value')
  await page.getByLabel('Sản phẩm').selectOption(productId!)
