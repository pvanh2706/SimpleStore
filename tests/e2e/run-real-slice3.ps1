...
$env:DevelopmentCashier__Email = "cashier-$runId@example.test"
$env:DevelopmentCashier__Password = "E2E-$runId!aA1"
$env:SIMPLESTORE_E2E_EMAIL = $env:DevelopmentOwner__Email
$env:SIMPLESTORE_E2E_PASSWORD = $env:DevelopmentOwner__Password
$env:SIMPLESTORE_E2E_CASHIER_EMAIL = $env:DevelopmentCashier__Email
$env:SIMPLESTORE_E2E_CASHIER_PASSWORD = $env:DevelopmentCashier__Password
...
try {
    ...
    pnpm --dir tests/e2e exec playwright test --config playwright.real.config.ts --grep 'Owner prepares Slice 3'
    if ($LASTEXITCODE -ne 0) { throw 'Could not prepare the real Slice 3 scenario.' }

    pnpm --dir tests/e2e exec playwright test --config playwright.real.config.ts --grep 'Cashier completes, prints and reprints'
    if ($LASTEXITCODE -ne 0) { throw 'The real Slice 3 Playwright flow failed.' }
}
finally {
